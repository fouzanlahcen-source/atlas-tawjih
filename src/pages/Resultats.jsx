...
export default function Resultats() {
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#F8FAFC', fontFamily:"'Segoe UI',sans-serif", display:'flex', flexDirection:'column' }}>
      ...
      <div style={{ flex:1, padding:'24px 20px', maxWidth:800, margin:'0 auto', width:'100%' }}>
        ...
        <div style={{ background:'#fff', border:'1px solid #E2E8F0', borderRadius:12, padding:'18px 20px', marginBottom:16, display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:12 }}>
          <div>
            <div style={{ fontSize:13, fontWeight:700, color:'#1E293B', marginBottom:4 }}>
              {lang==='fr' ? '📄 Télécharger le récapitulatif' : '📄 Download summary'}
            </div>
            <div style={{ fontSize:11, color:'#94A3B8' }}>
              {lang==='fr' ? '1 page · Infos · Scores · Plans · Contact' : '1 page · Info · Scores · Plans · Contact'}
            </div>
          </div>
          <button
            onClick={async () => {
              ...
              setPdfLoading(true)
              try { await genererPDF(eleve, scores, classement, lang) }
              catch(e) { alert('Erreur : ' + e.message) }
              finally { setPdfLoading(false) }
            }}
            disabled={pdfLoading}
            style={{ padding:'10px 24px', background:pdfLoading?'#94A3B8':`linear-gradient(135deg,${DIM_COLORS[classement[0]]},${DIM_COLORS[classement[0]]}CC)`, color:'#fff', border:'none', borderRadius:10, fontSize:13, fontWeight:700, cursor:'pointer', display:'flex', alignItems:'center', gap:8 }}>
            {pdfLoading ? (lang==='fr'?'⏳ Generation...':'⏳ Generating...') : (lang==='fr'?'⬇️ Telecharger':'⬇️ Download')}
          </button>
        </div>
  ...
  const BORDER=[226,232,240], LIGHT=[248,250,252], AMB=[217,119,6]
  ...
  // ── Header ──────────────────────────────────────────────
  fr(0,0,W,18,DRK)
  bx(ML,3,12,12,2,cA); sf(true,8,WHT); tx('AT',ML+6,11,'center')
  sf(true,11,WHT); tx('ATLAS TAWJIH',ML+17,11)
  sf(false,8,LGT); tx(lang==='fr'?'Recapitulatif Eleve':'Student Summary',ML+17,16)
  sf(false,7.5,LGT); tx(new Date().toLocaleDateString(lang==='fr'?'fr-FR':'en-US'),W-MR,11,'right')

  let y=26

  // ── Profil dominant + code ───────────────────────────────
  bx(ML,y,TW,28,3,mix(cA,0.08)); brd(ML,y,TW,28,3,mix(cA,0.3),0.5); fr(ML,y,4,28,cA)
  bx(ML+8,y+5,18,18,9,cA); sf(true,13,WHT); tx(classement[0],ML+17,y+16.5,'center')
  sf(true,14,DRK); tx(cl(profA.nom),ML+33,y+12)
  sf(false,8.5,GRY); tx(lang==='fr'?'Profil dominant':'Dominant profile',ML+33,y+19)
  sf(true,11,cA); tx(code3,ML+33,y+25)
  bx(W-MR-24,y+6,20,16,3,cA)
  sf(true,13,WHT); tx(`${scores[classement[0]]}%`,W-MR-14,y+16,'center')
  y+=35

  // ── Infos élève ──────────────────────────────────────────
  sf(true,10,DRK); tx(lang==='fr'?"Informations de l'eleve":'Student information',ML,y); y+=6
  dr(BORDER); doc.setLineWidth(0.4); doc.line(ML,y,W-MR,y); y+=4
  ...
  ;[
    ...
    [lang==='fr'?'Ecoles privees':'Private schools',eleve.prive==='oui'?(lang==='fr'?'Oui':'Yes'):(lang==='fr'?'Non':'No')],
    [lang==='fr'?'Date du test':'Test date',new Date().toLocaleDateString(lang==='fr'?'fr-FR':'en-US')],
    ...
    sf(false,9,GRY); tx(cl(String(v||'')),ML+TW*0.40,y+6.5)
    dr(BORDER); doc.setLineWidth(0.25); doc.line(ML,y+h,W-MR,y+h)
    y+=h
  })
  y+=8

  // ── Scores RIASEC — barres horizontales ─────────────────
  sf(true,10,DRK); tx(lang==='fr'?'Scores RIASEC':'RIASEC Scores',ML,y); y+=6
  dr(BORDER); doc.setLineWidth(0.4); doc.line(ML,y,W-MR,y); y+=5
  const allD=['R','I','A','S','E','C']
  allD.forEach((d,i)=>{
    const sc=scores[d], cc=RGB[d]
    sf(true,9,cc); tx(d,ML,y+4.5)
    sf(false,8.5,GRY); tx(cl(PROFILS[d][lang].nom),ML+9,y+4.5)
    bx(ML+58,y,TW-62,6,2,BORDER)
    bx(ML+58,y,Math.max(1,(TW-62)*sc/100),6,2,cc)
    sf(true,9,cc); tx(`${sc}%`,W-MR,y+5,'right')
    y+=10
  })
  y+=6

  // ── Plans A/B/C résumés ──────────────────────────────────
  sf(true,10,DRK); tx(lang==='fr'?'Plans d\'orientation':'Orientation plans',ML,y); y+=6
  dr(BORDER); doc.setLineWidth(0.4); doc.line(ML,y,W-MR,y); y+=5
  ;[[classement[0],profA,cA,lang==='fr'?'Plan A - Priorite':'Plan A - Priority'],
    [classement[1],profB,RGB[classement[1]],lang==='fr'?'Plan B - Alternative':'Plan B - Alternative'],
    [classement[2],profC,RGB[classement[2]],lang==='fr'?'Plan C - Repli':'Plan C - Backup'],
  ].forEach(([dim,prof,col,label])=>{
    bx(ML,y,TW,11,2,mix(col,0.1)); brd(ML,y,TW,11,2,mix(col,0.3),0.4); fr(ML,y,3,11,col)
    sf(true,7.5,mix(col,0.4)); tx(label,ML+6,y+5.5)
    sf(true,9.5,col); tx(`${dim} - ${cl(prof.nom)} (${scores[dim]}%)`,ML+6,y+9.5)
    // Domaines sur la même ligne
    const domsText=prof.domaines.slice(0,4).map(d=>cl(d)).join('  ·  ')
    sf(false,7.5,GRY); tx(domsText,ML+TW*0.42,y+9.5)
    y+=13
  })
  y+=8

  // ── Recommandations établissements ───────────────────────
  const recText=lang==='fr'
    ?`Les recommandations d'etablissements et de filieres specifiques seront communiquees directement par l'equipe Atlas Tawjih, en tenant compte du profil de ${cl(eleve.prenom)}, de sa ville, de ses preferences de mobilite et de son interet pour le prive. N'hesitez pas a nous contacter.`
    :`School and field recommendations will be provided directly by the Atlas Tawjih team, taking into account ${cl(eleve.prenom)}'s profile, city, mobility preferences and interest in private schools. Feel free to contact us.`
  const recLs=doc.splitTextToSize(cl(recText),TW-10), recH=recLs.length*5.5+14
  bx(ML,y,TW,recH,3,mix(cA,0.08)); brd(ML,y,TW,recH,3,mix(cA,0.3),0.5); fr(ML,y,3,recH,cA)
  sf(true,9,cA); tx(lang==='fr'?"A propos des recommandations d'etablissements":'About school recommendations',ML+7,y+7)
  sf(false,8.5,GRY); doc.text(recLs,ML+7,y+13)
  y+=recH+6

  // ── Contact ──────────────────────────────────────────────
  bx(ML,y,TW,18,3,DRK)
  sf(true,10,WHT); tx('Atlas Tawjih  |  0703244407',ML+8,y+7)
  sf(false,8.5,LGT); tx('atlastawjih.maroc@gmail.com  |  atlas-tawjih.vercel.app',ML+8,y+14)
  y+=24

  // ── Disclaimer ───────────────────────────────────────────
  const disc=lang==='fr'
    ?`Ce rapport a ete genere automatiquement par la plateforme Atlas Tawjih. Les resultats sont bases sur les reponses fournies par l'eleve lors du test RIASEC. Ce document est a titre indicatif uniquement.`
    :`This report was automatically generated by Atlas Tawjih. Results are based on the student's RIASEC test responses. For guidance purposes only.`
  const discLs=doc.splitTextToSize(cl(disc),TW)
  sf(false,7.5,LGT); doc.text(discLs,ML,y)

  // ── Footer ───────────────────────────────────────────────
  fr(0,H-11,W,11,DRK)
  sf(false,7.5,LGT); tx('Atlas Tawjih  |  atlastawjih.maroc@gmail.com',ML,H-4.5)
  sf(true,8.5,WHT); tx('1',W-MR,H-4.5,'right')

  doc.save(`AtlasTawjih_Recap_${cl(eleve.prenom)}_${cl(eleve.nom)}.pdf`)
}
